 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * QD2_MissionCtrl_Manual_2021a(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(QD2_MissionCtrl_Manual_2021a_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    pstream_shutdown(QD2_MissionCtrl_Manual_2021a_DW.StreamServer_Stream);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://QD2_MissionCtrl_Manual_2021a:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: QD2_MissionCtrl_Manual_2021a -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = QD2_MissionCtrl_Manual_2021a();
  if (rtmGetErrorStatus(S) != NULL) {
  ...
  string_copy((char *) QD2_MissionCtrl_Manual_2021a_B.ModelArgument, 80,
              extra_arg1);
