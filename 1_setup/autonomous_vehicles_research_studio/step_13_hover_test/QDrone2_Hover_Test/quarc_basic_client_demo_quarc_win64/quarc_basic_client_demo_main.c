...
typedef RT_MODEL_quarc_basic_client_d_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * quarc_basic_client_demo(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(quarc_basic_client_demo_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    pstream_shutdown(quarc_basic_client_demo_DW.StreamClient_Stream);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://quarc_basic_client_demo:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: quarc_basic_client_demo -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
