...
typedef RT_MODEL_QD2_DroneStack_PID_2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * QD2_DroneStack_PID_2021a(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(QD2_DroneStack_PID_2021a_M, GBLbuf.message);
}
...
static subrate_info_T subrate_info[3];
      ...
      if (local_result == 0) {
        local_result = timeout_subtract
          (&QD2_DroneStack_PID_2021a_DW.ComputationTime1_ComputationTim,
           &end_time, &QD2_DroneStack_PID_2021a_DW.ComputationTime1_BeginTime);
      }
      ...
      if (local_result == 0) {
        local_result = timeout_subtract
          (&QD2_DroneStack_PID_2021a_DW.ComputationTime2_ComputationTim,
           &end_time, &QD2_DroneStack_PID_2021a_DW.ComputationTime2_BeginTime);
      }
  ...
  main(int argc, char * argv[])
{
  ...
  int max_priority = qsched_get_priority_max(QSCHED_FIFO) - 3;
  qsigset_t signal_set;
  qsigaction_t action;
  int_T stack_size = 0;                /* default stack size */
  const char * extra_arg1 = "";
  ...
  for (count = 1; count < argc; ) {
    ...
    if ((strcmp(option, "-tf") == 0) && (count != argc)) {/* final time */
      ...
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    } else if (strcmp(option, "-URI_Host") == 0 && count != argc) {
      extra_arg1 = argv[count++];
      argv[count-2] = NULL;
      argv[count-1] = NULL;
    }
  }
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "tcpip://10.110.240.39:17001?retries=10");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: QD2_DroneStack_PID_2021a -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  string_copy((char *) QD2_DroneStack_PID_2021a_B.ModelArgument, 80, extra_arg1);
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      for (subrate_index = 2; subrate_index >= 0; --subrate_index) {
        ...
        if (result == 0) {
          ...
          if (result == 0) {
            ...
            if (result == 0) {
              ...
              {
                pstream_shutdown(QD2_DroneStack_PID_2021a_DW.StreamClient_Stream);
                pstream_shutdown(QD2_DroneStack_PID_2021a_DW.StreamServer_Stream);
              }

              /* Do NOT use the subrate_index variable for this loop! */
              for (subrate = 0; subrate < 3; subrate++) {
       ...
       */
      for (subrate_index++; subrate_index < 3; subrate_index++) {
